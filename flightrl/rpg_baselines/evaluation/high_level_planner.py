...
class HighLevelPlanner:
    def __init__(self,
        ...
        self.distance_to_avoid_corners = np.sqrt(
            self.world_box_dim_factors[0] ** 2 + self.world_box_dim_factors[1] ** 2 + (
                        self.world_box_dim_factors[2] / 2) ** 2) * 0.9
        ...
        for run_idx in range(num_runs):
            for goal_idx in range(num_goals_per_run):
                ...
                while not good_goal:
                    ...
                    if (min_distance_between_consecutive_goals < np.linalg.norm(
                            goals_relative_position) < self.distance_to_avoid_corners):
                        good_goal = True
    ...
    def get_current_goal(self, drone_position, num_run):
        ...
        if goal_relative_position_norm < self.switch_goal_distance:
            if self.internal_state == 4:  # arrived to last goal of episode
                ...
                safe_num_run = (num_run + 1) * (num_run < (self.num_runs - 1)) + num_run * (
                            num_run == (self.num_runs - 1))
