...
# create a class with an imposed API and put it int he same folder of this python script
# or specify the path relative to flightmare base folder
...
def main():
    ...
    object_density_fractions_different_episodes[(num_rollouts_per_density - 1):-1] = \
        object_density_fractions_different_episodes[num_rollouts_per_density:]
    ...
    while n_roll < num_rollouts:
        ...
        drone_pos = obs[0, :3]
        current_goal, _, _ = high_level_planner.get_current_goal(drone_position=drone_pos,
                                                                 num_run=int(n_roll % num_rollouts_per_density))
        ...
        while not (done or done_from_high_level_planner or (ep_len >= max_ep_length)):
            ...
            if ep_len == 5:
                env.set_objects_densities(
                    object_density_fractions=object_density_fractions_different_episodes[n_roll].reshape(1, -1))
            ...
            drone_pos = obs[0, :3]
            current_goal, done_from_high_level_planner, high_level_planner_goal_reached_number = \
                high_level_planner.get_current_goal(drone_position=drone_pos,
                                                    num_run=int(n_roll % num_rollouts_per_density))
    ...
    for density_idx in range(num_density_values):
        ...
        txt_file.write("\nepisodes_lengths               : \t")
        [txt_file.writelines(str("{:.7f}".format(episodes_lengths[n_roll]))[:9] + "\t") for n_roll in
         range(range_start, range_end)]

        txt_file.write("\nnumber_goal_reached            : \t")
        [txt_file.writelines(str("{:.7f}".format(episodes_terminal_goal_number[n_roll]))[:9] + "\t") for n_roll in
         range(range_start, range_end)]

    txt_file.close()

    pickle_file_path = os.path.join("./evaluation_result.pickle")
    # Save rollout info in a pickle file for later use.
    data_dict = {
    ...
    for density_idx in range(num_density_values):
        ...
        if range_end != num_density_values * num_density_values:
            ...
            mean_goal_reached_on_density_y_axis[density_idx] = np.mean(
                episodes_terminal_goal_number[range_start:range_end])
        else:
